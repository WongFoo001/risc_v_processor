...
// For the elf loader
resolvers += "Spring Plugins Repository" at "https://repo.spring.io/plugins-release/"
...
libraryDependencies ++= Seq(
  ...
  "edu.berkeley.cs" %% "chisel-iotesters" % "1.3.4"
  //"edu.berkeley.cs" %% "chiseltest" % "0.2-SNAPSHOT"
)
...
lazy val Mult = config("mult") extend(Test)
...
def multFilter(name: String): Boolean = name endsWith "TesterMult"
lazy val root = (project in file("."))
  .configs(TestAll).configs(Grader).configs(Lab1).configs(Lab2).configs(Lab3).configs(Lab4).configs(Mult)
  .settings(
    ...
    inConfig(Mult)(Defaults.testTasks),
    ...
    testOptions in Mult := Seq(Tests.Filter(multFilter))
  )