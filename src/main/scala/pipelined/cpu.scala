...
class PipelinedCPU(implicit val conf: CPUConfig) extends Module {
  ...
  // Temp Debug val 
  val pcWriteHaz = hazard.io.pcwrite
  val isMStall   = hazard.io.stage_mhold

  if (conf.debug) { printf(p"PC: $pc  || hazard.io.pcwrite: $pcWriteHaz\n") }
  if (conf.debug) { printf(p"   Multiplication Stall: $isMStall\n") }
  ...
  when (hazard.io.stage_mhold) { // Multiplication Stall
  ...
  id_ex.rs1        := rs1
  id_ex.rs2        := rs2
  id_ex.opcode     := if_id.instruction(6,0)   // Used for multiplication detection
  ...
  when (hazard.io.stage_mhold) { // Multiplication Stall
    // Fill the id_ex register with already stored values
    id_ex.writereg   := id_ex.writereg
    id_ex.rs1        := id_ex.rs1
    id_ex.rs2        := id_ex.rs2
    id_ex.opcode     := id_ex.opcode
    ...
    id_ex.branchpc   := id_ex.branchpc

    // Set the execution control signals with already stored control signals
    ...
    id_ex.excontrol.prediction := id_ex.excontrol.prediction

    // Set the memory control signals with already stored control signals
    ...
    // Set the writeback control signals with already stored control signals
    id_ex.wbcontrol.toreg    := id_ex.wbcontrol.toreg
  ...
  aluControl.io.opcode    := id_ex.opcode // Used to detect multiplication!

  // ALU multiplication start signal assigned here
  alu.io.multStart := aluControl.io.startMult
  ...
  alu.io.multStart    := aluControl.io.startMult    // Send multiplication start signal from ALU Control to the ALU
  alu.io.multFinished := aluControl.io.multFinished // Send multiplication finished signal from ALU Control to the ALU

  // Send multiplication stall signal to hazard unit
  hazard.io.mstall := aluControl.io.multHold // Stall to allow multiplication operation to complete
  ...
  when (hazard.io.stage_mhold) { // Multiplication Stall
    ...
    ex_mem.mcontrol   := ex_mem.mcontrol
  ...
  when (hazard.io.stage_mhold) {
    // Wire the MEM/WB register
    mem_wb.writereg   := mem_wb.writereg
    mem_wb.aluresult  := mem_wb.aluresult
    mem_wb.pcplusfour := mem_wb.pcplusfour
    mem_wb.readdata   := mem_wb.readdata
    mem_wb.wbcontrol  := mem_wb.wbcontrol
  }
